...
namespace Cocos2D
{
    ...
    public class CCBMFontConfig
    {
        ...
        internal List<string> _pages = new List<string>();
        public List<string> Pages { get=>_pages; }
        ...
        public CCBMFontConfig()
        {
        }

        public CCBMFontConfig(string data, string fntFile)
        {
        ...
        /// <returns></returns>
        public static CCBMFontConfig Create(string fntFile)
        {
            try
            {
                var pRet = CCContentManager.SharedContentManager.Load<CCBMFontConfig>(fntFile);
                if (pRet != null && !String.IsNullOrEmpty(pRet.AtlasName) && pRet.Pages.Count == 0)
                {
                    pRet.Pages.Add(Path.GetFileName(pRet.AtlasName));
                }
                return pRet;
            }
            catch (ContentLoadException)
            {
                var pRet = new CCBMFontConfig();
        ...
        public static CCBMFontConfig Create(Stream fntFile, string fntFileName)
        {
            var pRet = new CCBMFontConfig();
        ...
        private List<int> ParseConfigFile(string pBuffer, string fntFile)
        {
            ...
            Debug.Assert(pBuffer != null, "CCBMFontConfig::parseConfigFile | Open file error.");
                ...
                else if ((char.IsWhiteSpace(c) || i == (line.Length-1)) && mode == 1)
                {
                    ...
                    switch (token)
                    {
                        ...
                        case "xadvance":
                            ...
                            break;
                        case "page":
                            characterDefinition.page = Cocos2D.CCUtils.CCParseInt(value);
                            break;
                        case "chnl":
                            break;
                        case "letter":
